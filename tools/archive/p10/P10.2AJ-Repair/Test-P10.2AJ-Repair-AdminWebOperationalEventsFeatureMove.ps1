...
foreach ($script in $psScripts) {
    ...
    $badPatterns = @(
        ...
        [pscustomobject]@{ Name = 'Nested array validation'; Text = "@(`r`n    @(" },
        [pscustomobject]@{ Name = 'Corrupted tab escape'; Text = 'src\t' },
        [pscustomobject]@{ Name = 'Corrupted alert escape'; Text = 'src\a' }
    )

    foreach ($pattern in $badPatterns) {
        if ($text.Contains($pattern.Text)) {
            throw ('Unsafe script pattern found ({0}) in {1}' -f $pattern.Name, $script.FullName)
        }
    }
}

Write-Host 'P10.2AJ repair validation passed.'
