...
$docPath = Join-Path $docsRoot 'P10.2CN-Repair-AdminWebRuntimeApiSmokeMatrix.md'
$doc = New-Object System.Collections.Generic.List[string]
[void]$doc.Add('# P10.2CN Repair - Admin Web Runtime API Smoke Matrix')
[void]$doc.Add('')
[void]$doc.Add('Repair-only package for the optional runtime API smoke matrix runner.')
[void]$doc.Add('')
[void]$doc.Add('The runner now writes a summary immediately, uses per-request timeouts, limits endpoint probes, and always writes evidence files under `artifacts/p10/P10.2CN-Repair`.')
[void]$doc.Add('')
[void]$doc.Add('Run from the repository root:')
[void]$doc.Add('')
[void]$doc.Add('```powershell')
[void]$doc.Add('powershell -ExecutionPolicy Bypass -File .\tools\p10\P10.2CN-Repair\Run-P10.2CN-Repair-AdminWebRuntimeApiSmokeMatrix.ps1')
[void]$doc.Add('```')
Set-Content -Path $docPath -Value $doc -Encoding UTF8

Write-Host 'P10.2CN Repair Admin Web runtime API smoke matrix applied.'
