...
function Ensure-AppImport {
    param(
        ...
        [Parameter(Mandatory = $true)][string[]] $AnchorImports
    )
    ...
    foreach ($anchorImport in $AnchorImports) {
        if ($content.Contains($anchorImport)) {
            $replacement = $anchorImport + [Environment]::NewLine + $NewImport
    ...
    throw ('Unable to update App.tsx Operational Events import; no known old import or feature anchor was found in {0}' -f $Path)
}
...
Ensure-AppImport -Path $appFile -OldImport $oldImport -NewImport $newImport -AnchorImports $anchors
...
Write-Host 'P10.2AJ Repair2 applied.'
