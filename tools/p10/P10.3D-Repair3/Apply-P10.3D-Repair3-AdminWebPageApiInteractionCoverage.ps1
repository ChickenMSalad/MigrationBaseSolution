...
[void]$report.Add('# P10.3D Repair3 - Admin Web Page API Interaction Coverage')
[void]$report.Add('')
[void]$report.Add('Repairs the page API coverage runner so it only probes literal HTTP API paths that start with `/api/`.')
[void]$report.Add('')
[void]$report.Add('It skips relative module imports such as `../../../../api/client`, dynamic/template paths that cannot be safely parameterized, and non-GET interactions.')
[void]$report.Add('')
[void]$report.Add('Run:')
[void]$report.Add('')
[void]$report.Add('```powershell')
[void]$report.Add('powershell -ExecutionPolicy Bypass -File .\tools\p10\P10.3D-Repair3\Run-P10.3D-Repair3-AdminWebPageApiInteractionCoverage.ps1 -AdminApiBaseUrl "https://localhost:55436"')
[void]$report.Add('```')

Set-Content -Path $reportPath -Value $report.ToArray() -Encoding UTF8

[void][scriptblock]::Create((Get-Content -Path $runnerPath -Raw))

Write-Host ('Wrote report: {0}' -f $reportPath)
Write-Host 'P10.3D Repair3 Admin Web page API interaction coverage applied.'
