...
$sqlFile = Join-Path $repoRoot 'database\sql\p7\021_runtime_sql_baseline_reconciliation_validator.sql'

if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $repoRoot 'artifacts\runtime-sql-baseline\p7.9h-validator.txt'
}
...
Write-Host ('P7.9H validator output written to {0}' -f $OutputPath)
