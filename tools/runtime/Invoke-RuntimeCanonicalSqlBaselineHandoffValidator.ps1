...
if ([string]::IsNullOrWhiteSpace($SqlPath)) {
    $SqlPath = [System.IO.Path]::Combine($repoRoot, 'database\sql\p7\023_runtime_canonical_sql_baseline_handoff_validator.sql')
}
...
if (-not (Test-Path -LiteralPath $SqlPath)) {
    throw ('SQL validator file not found: {0}' -f $SqlPath)
}
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    ...
    $OutputPath = [System.IO.Path]::Combine($artifactRoot, 'p710b-sql-baseline-validator.txt')
}
...
if ($LASTEXITCODE -ne 0) {
    throw ('sqlcmd validator failed with exit code {0}. See {1}' -f $LASTEXITCODE, $OutputPath)
}

Write-Host ('P7.10B SQL baseline validator output written to {0}' -f $OutputPath)
