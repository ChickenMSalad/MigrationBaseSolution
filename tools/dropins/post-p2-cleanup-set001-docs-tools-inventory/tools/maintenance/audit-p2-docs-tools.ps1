...
try {
    ...
    if ($missingDocs.Count -eq 0) { $report += "- None" } else { $missingDocs | ForEach-Object { $report += "- $_" } }
    $report += ""
    $report += "## Missing expected tools"
    if ($missingTools.Count -eq 0) { $report += "- None" } else { $missingTools | ForEach-Object { $report += "- $_" } }
    $report += ""
    $report += "## Candidate cleanup targets"
    if ($cleanupTargets.Count -eq 0) { $report += "- None" } else { $cleanupTargets | ForEach-Object { $report += "- $_" } }
    $report += ""
