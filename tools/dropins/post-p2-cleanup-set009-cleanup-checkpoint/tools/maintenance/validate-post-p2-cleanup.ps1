...
try {
    ...
    foreach ($result in $auditResults) {
        $report += "| `$($result.Script)` | $($result.Status) |"
    }
    ...
    $report += ""
    $report += "If this number is greater than zero, review:"
    $report += ""
    $report += "```powershell"
    $report += "powershell -ExecutionPolicy Bypass -File .\tools\maintenance\remove-post-p2-dropin-artifacts.ps1"
    $report += "```"
    $report += ""
