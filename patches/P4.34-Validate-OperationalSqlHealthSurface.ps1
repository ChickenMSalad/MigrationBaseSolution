...
Assert-Contains `
    -Path (Join-Path $repoRoot "src/Core/Migration.Admin.Api/Endpoints/Operational/SqlHealth/OperationalSqlHealthEndpointExtensions.cs") `
    -Text "MapOperationalSqlHealthEndpoints"

Assert-OccursOnce `
    -Path $apiCompositionPath `
    -Text "endpoints.MapOperationalSqlHealthEndpoints();"

Assert-OccursOnce `
    -Path $uiCompositionPath `
    -Text "<OperationalSqlHealthWorkspace />"

Write-Host "[P4.34] Validation passed."
