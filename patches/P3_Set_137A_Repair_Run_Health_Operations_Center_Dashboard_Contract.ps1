...
$servicePath = Join-Path $repoRoot "src\Migration.Admin.Api\OperationalStore\Runs\Health\OperationalGlobalRunHealthOperationsCenterService.cs"

if (-not (Test-Path $servicePath)) {
    throw "Could not find $servicePath"
}

$backupPath = "$servicePath.137A.bak"
...
Write-Host "Repaired OperationalGlobalRunHealthOperationsCenterService.cs to match actual dashboard contract."
Write-Host ""
Write-Host "Next:"
Write-Host "  dotnet build"
Write-Host "  Restart Admin API"
Write-Host "  ./scripts/operational-global-run-health-operations-center-full-smoke-test.ps1 -BaseUrl `"https://localhost:55436`""
