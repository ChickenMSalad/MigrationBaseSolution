...
function Write-Step { param([string]$Message) Write-Host ("[P4.76] {0}" -f $Message) }
...
Copy-PayloadFile "src/Core/Migration.Admin.Api/Operational/Execution/ExecutionReplayAdmissionOptions.cs"
...
Copy-PayloadFile "config-samples/appsettings.ExecutionReplayAdmission.sample.json"
Copy-PayloadFile "docs/operations/P4.76-replay-admission-scheduling-governance.md"

$programPath = Join-Path $repoRoot "src/Core/Migration.Admin.Api/Program.cs"
Add-LineOnce -Path $programPath -Line "builder.Services.Configure<ExecutionReplayAdmissionOptions>(builder.Configuration.GetSection(ExecutionReplayAdmissionOptions.SectionName));" -Anchor "builder.Services.Configure"
Add-LineOnce -Path $programPath -Line "builder.Services.AddScoped<IExecutionReplayAdmissionService, SqlExecutionReplayAdmissionService>();" -Anchor "builder.Services.AddScoped<IExecutionReplayPolicyOverrideService, SqlExecutionReplayPolicyOverrideService>();"
...
Add-LineOnce -Path $compositionPath -Line "        endpoints.MapExecutionReplayAdmissionEndpoints();" -Anchor "        endpoints.MapExecutionReplayPolicyOverrideEndpoints();"
