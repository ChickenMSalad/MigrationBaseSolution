...
$testPath = Join-Path $repoRoot "scripts/workers/P4.57-Test-ExecutionWorkerHarness.ps1"
$docPath = Join-Path $repoRoot "docs/operations/P4.57-local-execution-worker-harness.md"

Assert-Contains -Path $workerPath -Text "ExecutionSessionId"
...
Assert-Contains -Path $testPath -Text "Worker harness validation passed"
Assert-Contains -Path $docPath -Text "Local Execution Worker Harness"

Write-Host "[P4.57] Validation passed."
