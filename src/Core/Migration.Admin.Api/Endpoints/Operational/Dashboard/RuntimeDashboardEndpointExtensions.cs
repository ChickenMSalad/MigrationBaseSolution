...
SELECT
    ...
    (SELECT COUNT_BIG(1) FROM migration.WorkItems WHERE Status IN (N'Running', N'InProgress', N'Processing', N'Started', N'Executing') OR (StartedAtUtc IS NOT NULL AND CompletedAtUtc IS NULL)) AS RunningWorkItemCount,
