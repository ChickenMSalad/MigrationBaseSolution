...
/// <summary>
/// Queue pressure trend endpoints composed from already-registered operational analytics services.
/// This avoids hard dependencies on unstable DTO contracts and returns available payloads plus trend metadata.
/// </summary>
public static class OperationalQueuePressureTrendEndpointExtensions
{
    public static RouteGroupBuilder MapOperationalQueuePressureTrendApi(this RouteGroupBuilder api)
    {
        ArgumentNullException.ThrowIfNull(api);

        api.MapGet(
                "/operational/queue-pressure/trend",
                async (int? sampleLimit, IServiceProvider serviceProvider, CancellationToken cancellationToken) =>
                {
                    ...
                    var queueDepthTrend = await TryInvokeOperationalServiceAsync(
                        serviceProvider,
                        serviceTypeNameContains: new[] { "GlobalQueueDepth", "QueueDepth" },
                        preferredMethodNameContains: new[] { "Trend", "History", "Sample", "Metrics", "Dashboard", "Analytics", "Summary" },
                        sampleLimit: limit,
                        cancellationToken);

                    var dispatcherTrend = await TryInvokeOperationalServiceAsync(
                        serviceProvider,
                        serviceTypeNameContains: new[] { "DispatcherPressure", "Dispatcher" },
                        preferredMethodNameContains: new[] { "Trend", "History", "Sample", "Pressure", "Dashboard", "Analytics", "Summary" },
                        sampleLimit: limit,
                        cancellationToken);
                    ...
                    return Results.Ok(new
                    {
                        trend = new
                        {
                            ...
                            readiness = BuildReadiness(queueDepthTrend, dispatcherTrend)
                        }
                    });
                })
            .WithName("GetOperationalQueuePressureTrend")
            .WithTags("Operational Store")
            .WithSummary("Gets a composed operational queue pressure trend view.")
            .Produces(StatusCodes.Status200OK);

        api.MapGet(
                "/operational/queue-pressure/trend/readiness",
            ...
            .WithName("GetOperationalQueuePressureTrendReadiness")
            .WithTags("Operational Store")
            .WithSummary("Gets queue pressure trend composition readiness.")
            .Produces(StatusCodes.Status200OK);

        return api;
    }

    private static object BuildReadiness(params OperationalServiceInvocationResult[] signals)
    {
        ...
        return new
        {
            ...
            isFullyComposed = availableSignalCount == signals.Length,
            missingSignals
        };
    }
