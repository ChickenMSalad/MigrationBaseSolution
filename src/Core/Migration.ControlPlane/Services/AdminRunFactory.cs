...
public sealed class AdminRunFactory
{
    ...
    private static bool RequiresManifest(MigrationProjectRecord project, IReadOnlyDictionary<string, string> settings)
    {
        if (!project.SourceType.Equals("SharePoint", StringComparison.OrdinalIgnoreCase))
        {
            return true;
        }

        var mode = GetSetting(settings, "SharePointMode", "SourceBinaryMode", "sourceService", "service") ?? "Rclone";
        return !mode.Equals("Rclone", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static string CreateId(string prefix) => $"{prefix}-{DateTimeOffset.UtcNow:yyyyMMddHHmmss}-{Guid.NewGuid():N}";

    private static string? NormalizeOptional(string? value) => string.IsNullOrWhiteSpace(value) ? null : value.Trim();

    private static string RequireResolvedPath(string? path, string name) => string.IsNullOrWhiteSpace(path)
