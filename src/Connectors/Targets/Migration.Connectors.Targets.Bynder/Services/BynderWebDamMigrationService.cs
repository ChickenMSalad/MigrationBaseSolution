...
{
    public class BynderWebDamMigrationService
    {
        private readonly ILogger<BynderWebDamMigrationService> _logger;
        ...
        private readonly WebDamApiClient _webDamApiClient;
        ...
        public BynderWebDamMigrationService(
            ExecutionContextState state,
            ILogger<BynderWebDamMigrationService> logger,
            ...
            WebDamApiClient webDamApiClient)
        {
            ...
            _webDamApiClient = webDamApiClient ?? throw new ArgumentNullException(nameof(webDamApiClient));
        ...
        public async Task UploadWebDamAssetsFromFile(CancellationToken cancellationToken)
        {
            ResetState();
            ...
            _logger.LogInformation("Processing {Count} rows from {Filename}.", fileData.Count(), _metadataFilename);

            foreach (var rowData in fileData)
            {
                cancellationToken.ThrowIfCancellationRequested();
                await ProcessWebDamAssetAsync(rowData, logOutput, cancellationToken);
            }

            SaveRowData();
            await LogToLocal("webdam_bynder_upload.xlsx", logOutput);
        }
        ...
        private async Task ProcessWebDamAssetAsync(
            Dictionary<string, string> rowData,
            List<string> logOutput,
            CancellationToken cancellationToken)
        {
            var filename = rowData["Filename"].Trim();
            var webDamId = rowData.TryGetValue("webdam_id", out var idValue)
                ? idValue?.Trim()
                : string.Empty;

            if (string.IsNullOrWhiteSpace(webDamId))
            {
                logOutput.Add($"processing failed: {filename} missing webdam_id");
                LogRowData(false, rowData, $"Processing failed: {filename} missing webdam_id");
                return;
            }

            var logLine = string.Join(",", rowData.Values);
            logOutput.Add($"Processing: {logLine}");
            _logger.LogInformation("Processing {Filename} from WebDam asset id {WebDamId}", filename, webDamId);

            var foundAssets = await _bynderClient.GetAssetService().GetMediaListAsync(new MediaQuery()
            {
                MetaProperties = new Dictionary<string, IList<string>>
                {
                    { "webdam_id", new List<string> { webDamId } }
                }
            });
            ...
            var queryStream = await GetQueryStreamFromWebDam(rowData, cancellationToken);
    ...
    ;
        ...
        private async Task<QueryStream> GetQueryStreamFromWebDam(
            Dictionary<string, string> rowData,
            CancellationToken cancellationToken)
        {
            QueryStream queryStream = new QueryStream();
            ...
            var filename = rowData["Filename"];
            var webDamId = rowData["webdam_id"];
            var mediaId = rowData.ContainsKey("Id") ? rowData["Id"] : string.Empty;
            string keywords = rowData["Keywords"];
            await using var sourceStream = await _webDamApiClient.DownloadAssetAsync(webDamId, cancellationToken);
            ...
            var metapropertyOptions = GetMetapropertyOptions(rowData);
            var description = rowData.ContainsKey("Description") ? rowData["Description"] : filename;
            ...
            if (!string.IsNullOrWhiteSpace(mediaId))
            {
                var mediaInfo = await _assetService.GetMediaInfoAsync(new MediaInformationQuery
                {
                    MediaId = mediaId
                });
            ...
            queryStream.Query = query;
            queryStream.Stream = seekableStream;
            queryStream.RowData = rowData;
        ...
        private async Task<Stream> GetSeekableWebDamStreamAsync(string webDamId, CancellationToken cancellationToken)
        {
            var tempPath = Path.GetTempFileName();

            await using var sourceStream = await _webDamApiClient.DownloadAssetAsync(webDamId, cancellationToken);
            await using (var tempWriteStream = new FileStream(tempPath, FileMode.Create, FileAccess.Write, FileShare.None))
            {
                await sourceStream.CopyToAsync(tempWriteStream, cancellationToken);
            }
