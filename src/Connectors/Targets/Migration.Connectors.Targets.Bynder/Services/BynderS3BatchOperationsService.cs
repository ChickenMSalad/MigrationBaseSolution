...
{
    public class BynderS3BatchOperationsService
    {
        private readonly ILogger<BynderS3BatchOperationsService> _logger;
        ...
        public BynderS3BatchOperationsService(
            ExecutionContextState state,
            ILogger<BynderS3BatchOperationsService> logger,
        ...
        public async Task UploadAssetsFromFile(CancellationToken cancellationToken)
        {
            ResetState();
            ...
            var keys = await _s3Storage.ListKeysAsync(prefix: null);

            _logger.LogInformation($"Found {keys.Count()} keys from S3.");

            string startAt = "AIRMKT 2021 Atlas Interior Lux White _69271 PRINT.tif";
            bool bStarted = false;
            foreach (var rowData in fileData)
            {
                //if (rowData[Filename] == startAt)
                //{
                //    bStarted = true;
                //}

                //if (bStarted)
                    await ProcessAssetAsyncFromFile(rowData, keys, logOutput);
            ...
            await LogToLocal("some_bynder_upload.xlsx", logOutput);
        ...
        private async Task ProcessAssetAsyncFromFile(Dictionary<string, string> rowData, IReadOnlyList<string> keys, List<string> logOutput)
        {
            var filename = rowData["Filename"].Trim();
            var originId = rowData["EmbedId"];

            // HANDLE REQUIRED STUFF
            var dateAdded = rowData["DateAdded_DT"];

            DateTime dateTime = DateTime.Parse(dateAdded);
            string isoUtc = dateTime.ToUniversalTime().ToString("o");
            rowData["Date_Added_to_Widen"] = isoUtc;

            rowData["Asset_Status"] = "Unreleased";
            rowData["Asset_Purpose"] = "DAM_only";

            var modelYear = rowData["Model_Year"];
            if (string.IsNullOrEmpty(modelYear))
            {
                rowData["Model_Year"] = "Evergreen";
            }

            var assetType = rowData["Asset_Type"];
            if (string.IsNullOrEmpty(assetType))
            {
                rowData["Asset_Type"] = "Photo";
            }

            var assetSubType = rowData["Asset_Sub-Type"];
            if (string.IsNullOrEmpty(assetSubType))
            {
                rowData["Asset_Sub-Type"] = "Marketing_Photos";
            }
            ...
            {
                rowData["Product"] = "Travel_Trailers";
            }
            ...
            else
            {
                ...
                try
                {
                    key = keys.Where(k => Path.GetFileName(k).Equals(filename,StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
                    if (key != null)
                    {
                        var bExists = await _s3Storage.ExistsAsync(key);
                        logOutput.Add($"S3 Filename Exists at : {key} : {bExists}");
                        _logger.LogInformation($"S3 Filename Exists at : {key} : {bExists}");
                        if (!bExists)
                        {
                            logOutput.Add($"processing failed: {filename} does not exist in S3 assets");
                            LogRowData(false, rowData, $"Processing failed: {filename} does not exist in S3 assets");
                            return;
                        }
                    ...
                    } else
                    {
                        ...
                        _logger.LogInformation($"WHOA! Cannot find Key for : {filename}.  going to try to match without extension...");
                        key = keys.Where(k => Path.GetFileNameWithoutExtension(k).Equals(Path.GetFileNameWithoutExtension(filename), StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
                        ...
                        if (key != null)
                        {
                            ...
                            {
                                rowData["Filename"] = Path.GetFileName(key);
                            }
                        } else
                        {
                            _logger.LogInformation($"WHOA WHOA WHOA! Still Cannot find Key for : {filename}.  going to try to match by adding extension...");
                            key = keys.Where(k => Path.GetFileName(k).Equals(filename + ".tif", StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
                            ...
                            if (key != null)
                            {
                                ...
                                {
                                    rowData["Filename"] = Path.GetFileName(key.Replace(".","_")) + ".tif";
                                }
                            } else
                            {
                                _logger.LogInformation($"Cannot find Key for : {filename}");
                                logOutput.Add($"Cannot find Key for : {filename}");
                                LogRowData(false, rowData, $"Cannot find Key for : {filename}");
                                return;
                            }
                ...
                try
                {
                    QueryStream queryStream = await GetQueryStreamFromS3(rowData, key);
                    var response = await _assetResiliencyService.UploadFileAsync(queryStream.Stream, queryStream.Query);
                    rowData["Id"] = response.MediaId;
                    LogRowData(response.IsSuccessful, rowData, $"{filename} creation success: {response.IsSuccessful}");
        ...
        public async Task Test()
        {
            ...
            var logOutput = new List<string>();
            _logger.LogInformation("This is just a test");

            var keys = await _s3Storage.ListKeysAsync(prefix: null);

            foreach(string key in keys)
            {
                string k = Path.GetFileName(key);
            ...
            //await LogToAzure("test_bynder_take1.xlsx", logOutput);
    ...
    ;
        ...
        private async Task<QueryStream> GetQueryStreamFromS3(Dictionary<string, string> rowData, string key)
        {
            QueryStream queryStream = new QueryStream();
            var filename = rowData["Filename"];

            long size = await _s3Storage.GetAssetSizeAsync(key);
            Stream stream = null;
            // Stream is greater than 800MB
            if (size > _maxBytes)
            {
                await _s3Storage.DownloadToFileAsync(key, $"{_tempDirectory}{filename}");
                stream = new FileStream($"{_tempDirectory}{filename}", FileMode.Open, FileAccess.Read);//await _assetsWrapper.DownloadBlobAsync(azureFilename, folderName);
            } else
            {
                stream = await _s3Storage.OpenReadAsync(key);
            }

            var metapropertyOptions = GetMetapropertyOptions(rowData);
            var description = rowData["Description"];
            var mediaId = rowData["Id"];
            string keywords = rowData["Tags"];
            UploadQuery query = new()
            {
                Filepath = filename,
                Description = description,
