...
namespace Migration.Hosts.WebDamToBynder.Functions.QueueFunctions;
    ...
    ILogger<AssetQueueFunction> logger)
{
    ...
    public async Task Run([QueueTrigger("webdam-bynder-jobs", Connection = "AzureWebJobsStorage")] string message)
    {
        ...
        {
            await bynderS3BatchOperationsService.UploadAssetsFromMetadata(blobName, stream, take);
        }
        else if (blobName.StartsWith("update_bynder_", StringComparison.OrdinalIgnoreCase))
        {
            ...
            await bynderS3UpdateOperationsService.UpdateAssetsFromMetadata(blobName, stream, tableClient, take);
        }
