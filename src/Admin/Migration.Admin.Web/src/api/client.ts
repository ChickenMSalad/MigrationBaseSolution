...
export const api = {
  ...
  bindProjectArtifacts: (projectId: string, payload: BindProjectArtifactsRequest) =>
    request(`/api/projects/${encodeURIComponent(projectId)}/artifacts`, { method: "PUT", body: JSON.stringify(payload) }),
  bindProjectCredentials: (projectOrProjectId: ProjectRecord | string, payload: BindProjectCredentialsRequest) => {
    ...
    return request(`/api/projects/${encodeURIComponent(projectId)}/credentials`, { method: "PUT", body: JSON.stringify(payload) });
  },

  artifacts: (kind?: string) => request<ArtifactRecord[]>(`/api/artifacts${queryString({ kind })}`),

  // IMPORTANT: /api/artifacts/{artifactId} returns artifact metadata JSON.
  // Downloads must use the explicit download endpoint.
  ...
  buildManifest: (payload: BuildManifestRequest) => request<BuildManifestResponse>("/api/manifest-builder/build", { method: "POST", body: JSON.stringify(payload) }),
  createManifestArtifact: (payload: BuildManifestRequest) => request<BuildManifestResponse>("/api/manifest-builder/build", { method: "POST", body: JSON.stringify(payload) }),
  ...
  queuePreflight: (projectId: string) => request<RunRecord>(`/api/projects/${encodeURIComponent(projectId)}/preflight`, { method: "POST" }),
