...
function Add-SearchSection {
    ...
    $Lines.Add('') | Out-Null
    $folder = Join-Path $root $SearchRoot
...
$out = Join-Path $root 'docs\p8\P8.5A-Operational-Telemetry-Smoke-Execution-Inventory.generated.md'
$Lines = New-Object System.Collections.Generic.List[string]

$Lines.Add('# P8.5A Operational Telemetry Smoke Execution Inventory') | Out-Null
$Lines.Add('') | Out-Null
$Lines.Add(('GeneratedUtc: {0:O}' -f [DateTimeOffset]::UtcNow)) | Out-Null
$Lines.Add('') | Out-Null
$Lines.Add('This inventory verifies the runtime telemetry smoke-test surfaces before local or Azure Monitor proof-of-life execution.') | Out-Null
...
Add-FileSummary -RelativePath 'src\Core\Migration.Application\Operational\Telemetry\OperationalExecutionActivity.cs' -Patterns @('StartSqlQueueWorkItemExecution', 'StartServiceBusWorkItemExecution', 'StartServiceBusDispatch', 'SetExecutionDuration', 'SetExecutionResult')
...
Add-FileSummary -RelativePath 'config\templates\p8.5a-operational-telemetry-smoke-settings.template.json' -Patterns @('EnableTracing', 'EnableAzureMonitorExporter', 'TraceSamplingRatio')

Add-SearchSection -Title 'Runtime Activity usage' -SearchRoot 'src' -Pattern 'OperationalExecutionActivity.'
Add-SearchSection -Title 'OpenTelemetry registrations' -SearchRoot 'src' -Pattern 'AddOperationalOpenTelemetry'
Add-SearchSection -Title 'Azure Monitor exporter registrations' -SearchRoot 'src' -Pattern 'AddAzureMonitorTraceExporter'
