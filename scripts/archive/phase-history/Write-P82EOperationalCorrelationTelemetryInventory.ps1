...
$outputPath = Join-Path $root 'docs\p8\P8.2E-Operational-Correlation-Telemetry-Inventory.generated.md'
...
$lines.Add('# P8.2E Operational Correlation / Telemetry Inventory') | Out-Null
$lines.Add('') | Out-Null
$lines.Add(('GeneratedUtc: {0:O}' -f [DateTime]::UtcNow)) | Out-Null
$lines.Add('') | Out-Null
$lines.Add('This inventory captures repo-native distributed execution correlation and telemetry surfaces after P8.2C.') | Out-Null
$lines.Add('') | Out-Null
...
Add-FileSummary -Lines $lines -RootPath $root -RelativePath 'src\Workers\Migration.Workers.ServiceBusExecutor\Runtime\SqlServiceBusExecutorWorker.cs' -Patterns @('ServiceBusProcessor', 'CompleteMessageAsync', 'DeadLetterMessageAsync', 'AbandonMessageAsync')
Add-FileSummary -Lines $lines -RootPath $root -RelativePath 'src\Workers\Migration.Workers.QueueExecutor\Services\SqlOperationalWorkItemWorker.cs' -Patterns @('IOperationalExecutionHistoryWriter', 'ExecuteClaimedItemAsync', 'CompleteOperationalWorkItemRequest', 'FailOperationalWorkItemRequest')
...
Add-SearchSection -Lines $lines -RootPath $root -Title 'RunId structured references' -Pattern 'RunId={'
Add-SearchSection -Lines $lines -RootPath $root -Title 'WorkItemId structured references' -Pattern 'WorkItemId={'
...
Add-SearchSection -Lines $lines -RootPath $root -Title 'Operational execution history references' -Pattern 'IOperationalExecutionHistoryWriter'
