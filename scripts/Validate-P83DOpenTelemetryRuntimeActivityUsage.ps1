...
Assert-PathExists -RootPath $root -RelativePath 'docs\p8\P8.3D-OpenTelemetry-Runtime-Activity-Usage.md'
...
Assert-FileContains -RootPath $root -RelativePath 'src\Workers\Migration.Workers.QueueExecutor\Services\SqlOperationalWorkItemWorker.cs' -Text 'OperationalExecutionActivity.StartSqlQueueWorkItemExecution'
Assert-FileContains -RootPath $root -RelativePath 'src\Workers\Migration.Workers.QueueExecutor\Services\SqlOperationalWorkItemWorker.cs' -Text 'OperationalExecutionActivity.SetExecutionDuration'
Assert-FileContains -RootPath $root -RelativePath 'src\Workers\Migration.Workers.ServiceBusExecutor\Runtime\SqlServiceBusExecutorWorker.cs' -Text 'OperationalExecutionActivity.StartServiceBusWorkItemExecution'
Assert-FileContains -RootPath $root -RelativePath 'src\Workers\Migration.Workers.ServiceBusExecutor\Runtime\SqlServiceBusExecutorWorker.cs' -Text 'OperationalExecutionActivity.SetExecutionResult'
Assert-FileContains -RootPath $root -RelativePath 'src\Workers\Migration.Workers.ServiceBusDispatcher\Dispatching\SqlWorkItemDispatcher.cs' -Text 'OperationalExecutionActivity.StartServiceBusDispatch'

Write-Host 'P8.3D OpenTelemetry runtime Activity usage validation passed.'
